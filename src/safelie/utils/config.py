...
class AttackConfig(BaseModel):
    ...
    # P1 (docs/p1_concentrated_attack_gates.md). WHICH CONSTRAINT OWNERS
    # the compromised reporting channel delivers to, by agent id.
    #
    # `None` (the default, and the only value any pre-P1 config carries)
    # keeps the historical semantics exactly: the corrupted source is
    # corrupted in EVERY owner's report bundle. Under the G9+ trajectory-
    # batch architecture one replica reports a value to all N owners, so
    # that historical setting injects the SAME per-owner perturbation into
    # all N dual coordinates -- measured on the committed A1 logs as
    # delta_k = -(B/M) * 1 exactly, to 9.8e-15, on every round. That is a
    # uniform perturbation, and it is therefore NOT an instance of the
    # concentrated delta_k = delta * e_j that Proposition `cor:spread`
    # (main_iclr.tex) is about: A1/A2/A3 never instantiated the spreading
    # mechanism, they instantiated its already-uniform limit.
    #
    # Naming owners here restricts the corruption to the listed owners'
    # report bundles and leaves every other owner's bundle untouched,
    # giving delta_k = -(B/M) * e_j for a single named owner. The threat
    # model is UNCHANGED in kind and is if anything weaker: the same
    # static, persistent, directional, consistent f=1 adversary, holding
    # one reporting CHANNEL (source -> owner j) rather than one source
    # outright. This is the compromised-channel injection point
    # `safelie.attacks.static` already documents as load-bearing; it is a
    # restriction of the corruption's scope, not a new operator.
    #
    # Validated at construction: every id must be a real agent id for this
    # environment's N, with no duplicates and no empty list.
    corrupted_owner_ids: list[str] | None = None
...
class ExperimentConfig(BaseModel):
    ...
    def _cross_field_checks(self) -> ExperimentConfig:
        ...
        # P1. An explicit owner set must name real agents. The agent ids
        # are generated by the env factory as `agent_0..agent_{N-1}`, so
        # they are checkable here without constructing the environment.
        if self.attack.corrupted_owner_ids is not None:
            owners = self.attack.corrupted_owner_ids
            valid = {f"agent_{i}" for i in range(self.env.n_agents)}
            if not owners:
                raise ValueError(
                    "attack.corrupted_owner_ids is an empty list. Omit the field "
                    "entirely for the historical all-owner behaviour; an empty "
                    "list would silently mean 'attack nobody' while the config "
                    "still declares f>0."
                )
            unknown = [a for a in owners if a not in valid]
            if unknown:
                raise ValueError(
                    f"attack.corrupted_owner_ids names agents that do not exist: "
                    f"{unknown}. Valid agent ids for env.n_agents="
                    f"{self.env.n_agents} are {sorted(valid)}."
                )
            if len(set(owners)) != len(owners):
                raise ValueError(
                    f"attack.corrupted_owner_ids contains duplicates: {owners}"
                )
            if self.attack.name == "none":
                raise ValueError(
                    "attack.corrupted_owner_ids is set but attack.name is 'none'; "
                    "a config must not carry an adversary scope that nothing reads."
                )
