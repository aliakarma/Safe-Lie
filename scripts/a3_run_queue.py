#!/usr/bin/env python
"""A3 production run queue -- sequential, gated, resumable. Mac mini only.

Runs all twelve A3 runs and enforces docs/a3_gates.md section 7 after each,
...
**All twelve on one machine (BRANCH-B', gates doc section 12.4).** The
cross-platform probe showed the two machines do not agree bitwise -- torch
initialises different weights on arm64 -- so every contrast must live on one
machine. Putting all of A3 on the faster one gives three seeds for the same
wall clock as two split across both, and removes the machine factor entirely
...
**Sequential, not parallel.** The source collector already saturates the
machine; two concurrent runs would not finish sooner and would make the
per-run stop rule unusable.

Usage:
    python scripts/a3_run_queue.py
    python scripts/a3_run_queue.py --only C_seed0
    python scripts/a3_run_queue.py --dry-run
"""
...
STATUS = OUT_ROOT / "a3_queue_status.json"
...
def main() -> int:
    ...
    state = {"started": now(), "machine": "mac-mini (BRANCH-B')",
