...
| **Comparative Baselines** | Partly deferred | PID-Lagrangian is **implemented** as a selectable multiplier controller (`dual.controller: pid`), with gains deliberately left undeclared — see [`src/safelie/training/dual.py`](src/safelie/training/dual.py). MACPO and Dec-PDO remain deferred to Stage-3 after Stage-2 pilot completion. |
