...
### A3 production: `workers` 30 -> 20, all three seeds concurrent

The twelve A3 production configs move to `source_collection.workers: 20`.
This is a **compute-only** change and it is the last change before the GCP
campaign launches; no run exists yet on any machine.

**What it buys.** 3 x 20 = 60 vCPU, so all three seeds run concurrently on
the one `t2d-standard-60` instead of seeds 0 and 1 at 30 workers followed by
seed 2 alone. The old schedule left 30 of 60 cores idle for an entire phase
in which seed 2 did a third of the campaign's work; projected wall clock goes
from ~33-42 h to ~24-29 h.

**What it costs, stated.** 20 does not divide the 150-trajectory round
exactly: `ceil(150/20) = 8` waves against 7.5 ideal, roughly 6%, where 30
divided it into exactly 5. Expect ~67 s/round rather than ~42 s/round. It
also leaves no core headroom, which promotes the `OMP/MKL/OPENBLAS` thread
settings in the runbook from advisable to mandatory.

**Why it cannot touch a result.** `workers` selects the chunk partition and
nothing else -- every `(env_seed, torch_seed)` pair is drawn in the main
process before dispatch, and each trajectory reseeds env and torch at its own
top. `docs/a3_gates.md` §15.3 licensed that with a sweep over
`{1, 2, 4, 5, 6, 8, 12}`, which contained neither 30 nor 20. The sweep was
therefore **extended before this change was committed**, to
`{1, 2, 4, 5, 6, 8, 12, 20, 30}` at M=5, at `R_m=8` and at production
`R_m=30`, across chunk partitions from 40 to 120: all 30 source means and
every per-trajectory `G_r^i` bitwise identical, max abs difference exactly
0.0. The production value is now a measured point, not an interpolation.

Recorded as `docs/a3_gates.md` §16, an addendum -- §§1-14 and §15 are
unedited, per the amendment convention §15 itself followed. §15.4's "Seed 2
is not claimed to be concurrent" is superseded in that one respect and left
in place. Nothing scientific moves: `M=5`, `f=1`, `beta=1.5`, `R_m=30`,
`chunks_per_worker=4`, `R_ref=120`, the validation rounds, the entropies, the
attacked-source mapping, the attack and RCE blocks, PPO, the dual update and
the 250-round horizon are all untouched, and `scripts/a3_verify_frozen.py`
still refuses every `source_collection` field except `M`, `seed_entropy` and
`workers`.

Also updated: `docs/a3_gcp_runbook.md` §§2/4/6 (launch is now three
concurrent queues), `scripts/a3_run_queue.py`'s usage docstring, and
`tests/unit/test_a3_verify_frozen_workers.py`, which pins the production
value and now pins it at 20.

### A3 queue bookkeeping is no longer tracked, and the test suite stops eating it

Found while preparing the launch commit: `results/runs_a3/a3_queue_status_seed{0,1,2}.json`
were tracked, and **both** halves of the runbook's own pre-flight modified
them moments before launch. `test_a3_queue_isolation.py`'s `clean_status`
fixture deleted them outright, and `a3_run_queue.py --dry-run` rewrote them.
Since `safelie.experiment._git_sha` stamps `dirty` and `dirty_paths` into
every `run_metadata.json`, the documented pre-flight sequence guaranteed that
all twelve production runs would record a dirty tree.

Two fixes, neither of which touches a scientific parameter:

* **Untracked and gitignored** the per-seed status files, the `.a3_queue*.lock`
  lock files and the `*.tmp` atomic-write temporaries. These are live
  scheduler state, rewritten after every run and regenerated by any
  invocation; the campaign's actual audit trail is each run's own
  `rounds.jsonl`, `run_metadata.json` and gate reports, all still tracked.
  `a3_frozen_verification.json` also stays tracked -- it is a verification
  result, not scheduler state.
* **`clean_status` now saves and restores** instead of deleting, matching the
  pattern `test_unfiltered_queue_still_covers_all_twelve_and_uses_the_original_file`
  already used for `a3_queue_status.json`. The gitignore alone would have
  settled the provenance problem, but a fixture that destroys a halted
  campaign's gate outcomes and halt reasons -- the very thing
  `write_status`'s docstring calls the audit trail -- is a trap whatever git
  thinks of it.

`docs/a3_gcp_runbook.md` §3 gains a fifth pre-flight step, `git status
--porcelain`, explicitly ordered last because steps 2 and 4 are writers, and
required to print nothing.
